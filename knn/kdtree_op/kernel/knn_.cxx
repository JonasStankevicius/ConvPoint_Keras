...
void cpp_knn(const float* points, const size_t npts, const size_t dim, 
			...
			const size_t K, long long* indices, std::string& errorMsg){

	// create the kdtree
	typedef KDTreeTableAdaptor< float, float> KDTree;
	KDTree mat_index(npts, dim, points, errorMsg, 10);
	...
	for(size_t i=0; i<nqueries; i++){
		mat_index.query(&queries[i*dim], K, &out_ids[0], &out_dists_sqr[0], errorMsg);
...
void cpp_knn_batch(const float* batch_data, const size_t batch_size, const size_t npts, const size_t dim,
			const float* queries, const size_t nqueries,
			const size_t K, long long* batch_indices, std::string& errorMsg){
	...
	for(size_t bid=0; bid < batch_size; bid++){
		...
		// iterate over the points
		for(size_t i=0; i<nqueries; i++){			
			mat_index.query(&queries[bid*nqueries*dim + i*dim], K, &out_ids[0], &out_dists_sqr[0], errorMsg);
				...
				const size_t K, long long* batch_indices, std::string& errorMsg)
{
	...
	for(size_t bid=0; bid < batch_size; bid++){
		...
		// create the kdtree
		typedef KDTreeTableAdaptor< float, float> KDTree;
		KDTree tree(npts, dim, points, errorMsg, 10);
		...
		for(size_t ptid=0; ptid<nqueries; ptid++)
		{
			...
			tree.query(&query[0], K, &ids[0], &dists[0], errorMsg);
